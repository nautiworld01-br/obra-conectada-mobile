...
alter table public.daily_log_service_items
  drop constraint if exists daily_log_service_items_status_check;

alter table public.daily_log_service_items
  add constraint daily_log_service_items_status_check
...
begin
  ...
  if v_no_work_reason is null and jsonb_array_length(coalesce(p_service_items, '[]'::jsonb)) > 0 then
    insert into public.daily_log_service_items (log_id, room_id, description, status, order_index)
    select
      v_log_id,
