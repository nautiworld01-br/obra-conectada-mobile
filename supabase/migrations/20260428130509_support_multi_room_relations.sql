create table if not exists public.daily_log_rooms (
  id uuid primary key default gen_random_uuid(),
  log_id uuid not null references public.daily_logs(id) on delete cascade,
  room_id uuid not null references public.rooms(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (log_id, room_id)
);

alter table public.daily_log_rooms enable row level security;

create policy "Members can view log rooms"
  on public.daily_log_rooms for select to authenticated
  using (
    exists (
      select 1
...
create policy "Writers can create log rooms"
  on public.daily_log_rooms for insert to authenticated
  with check (
    exists (
      select 1
      from public.daily_logs as logs
...
create policy "Writers can delete log rooms"
  on public.daily_log_rooms for delete to authenticated
  using (
    exists (
      select 1
      from public.daily_logs as logs
...
create index if not exists daily_log_rooms_log_id_idx on public.daily_log_rooms (log_id);
create index if not exists daily_log_rooms_room_id_idx on public.daily_log_rooms (room_id);

create table if not exists public.weekly_update_rooms (
  id uuid primary key default gen_random_uuid(),
  update_id uuid not null references public.weekly_updates(id) on delete cascade,
  room_id uuid not null references public.rooms(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (update_id, room_id)
);

alter table public.weekly_update_rooms enable row level security;

create policy "Members can view update rooms"
  on public.weekly_update_rooms for select to authenticated
  using (
    exists (
      select 1
      from public.weekly_updates as updates
      where updates.id = update_id
        and public.is_member_of_project(updates.project_id)
    )
  );

create policy "Writers can create update rooms"
  on public.weekly_update_rooms for insert to authenticated
  with check (
    exists (
      select 1
      from public.weekly_updates as updates
      where updates.id = update_id
        and public.can_write_project(updates.project_id)
    )
  );

create policy "Writers can delete update rooms"
  on public.weekly_update_rooms for delete to authenticated
  using (
    exists (
      select 1
      from public.weekly_updates as updates
...
create index if not exists weekly_update_rooms_update_id_idx on public.weekly_update_rooms (update_id);
create index if not exists weekly_update_rooms_room_id_idx on public.weekly_update_rooms (room_id);

insert into public.daily_log_rooms (log_id, room_id)
select logs.id, logs.room_id
from public.daily_logs as logs
where logs.room_id is not null
on conflict (log_id, room_id) do nothing;

insert into public.weekly_update_rooms (update_id, room_id)
select updates.id, updates.room_id
from public.weekly_updates as updates
where updates.room_id is not null
on conflict (update_id, room_id) do nothing;
...
begin
  ...
  into v_room_ids
  from (
    select distinct on (input.room_id)
...
drop function if exists public.upsert_weekly_update_with_rooms(
  uuid,
  uuid,
  uuid,
  text,
