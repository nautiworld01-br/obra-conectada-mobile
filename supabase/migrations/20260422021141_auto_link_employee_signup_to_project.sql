...
begin
  ...
  values (
    ...
    v_is_employee,
    'ativo',
    case when v_is_employee then 'Funcionário' else null end
  )
  ...
  set
    ...
    occupation_role = coalesce(public.profiles.occupation_role, excluded.occupation_role);
...
begin
  ...
  if v_project_count = 1 and v_project_id is not null then
    ...
    where profiles.is_employee = true
      and not exists (
        select 1
