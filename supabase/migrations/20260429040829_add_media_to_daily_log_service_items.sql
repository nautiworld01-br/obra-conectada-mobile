alter table public.daily_log_service_items
  add column if not exists photos_urls jsonb;

alter table public.daily_log_service_items
  add column if not exists videos_urls jsonb;
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_service_items, '[]'::jsonb)) with ordinality as input(item, ordinality)
    where coalesce(nullif(input.item ->> 'status', ''), 'em_andamento') not in ('pendente', 'em_andamento', 'concluido')
  ) then
    raise exception 'Status de frente inválido.';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_service_items, '[]'::jsonb)) with ordinality as input(item, ordinality)
    where input.item ? 'photos_urls'
      and input.item -> 'photos_urls' <> 'null'::jsonb
      and jsonb_typeof(input.item -> 'photos_urls') <> 'array'
  ) then
    raise exception 'Fotos por frente inválidas. Esperado um array JSON.';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(coalesce(p_service_items, '[]'::jsonb)) with ordinality as input(item, ordinality)
    where input.item ? 'videos_urls'
      and input.item -> 'videos_urls' <> 'null'::jsonb
      and jsonb_typeof(input.item -> 'videos_urls') <> 'array'
  ) then
    raise exception 'Vídeos por frente inválidos. Esperado um array JSON.';
  end if;
  ...
  if v_no_work_reason is null and jsonb_array_length(coalesce(p_service_items, '[]'::jsonb)) > 0 then
    insert into public.daily_log_service_items (
      log_id,
      room_id,
      description,
      status,
    ...
    select
      ...
      coalesce(nullif(input.item ->> 'status', ''), 'em_andamento'),
      input.ordinality - 1,
      case
        when input.item -> 'photos_urls' is null or input.item -> 'photos_urls' = '[]'::jsonb then null
        else input.item -> 'photos_urls'
      end,
      case
        when input.item -> 'videos_urls' is null or input.item -> 'videos_urls' = '[]'::jsonb then null
        else input.item -> 'videos_urls'
      end
    from jsonb_array_elements(coalesce(p_service_items, '[]'::jsonb)) with ordinality as input(item, ordinality);
  end if;
