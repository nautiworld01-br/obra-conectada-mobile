...
function StageForm({
  ...
  const applyDate = (iso: string) => {
    ...
    if (activeDateField === "plannedStart") {
      setPlannedStart(display);
    }

    if (activeDateField === "plannedEnd") {
      setPlannedEnd(display);
    }
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalContent}>
            ...
            <Pressable style={({ pressed }) => [styles.primaryButton, (loading || pressed) && styles.buttonPressed]} onPress={() => void handleSave()}>
              {loading ? (
                ...
                <Text style={styles.primaryButtonText}>{stage ? "Salvar Alteracoes" : "Criar Etapa"}</Text>
              )}
            </Pressable>
...
export function ScheduleScreen() {
  ...
  const handleOpenEdit = (stage: StageRow) => {
  ...
  const handleDelete = () => {
    ...
    Alert.alert("Excluir etapa?", "Essa etapa sera removida do cronograma.", [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          void deleteStage.mutateAsync({ id: selectedStage.id, projectId: project.id }).then(() => {
            setFormOpen(false);
            setSelectedStage(null);
          });
        },
    ...
    <View style={styles.screen}>
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>Nenhuma etapa cadastrada ainda. Adicione as etapas da sua obra.</Text>
        </View>
      ) : (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          <View style={styles.summaryRow}>
            {[
              ...
              <Pressable
                key={item.label}
                style={({ pressed }) => [styles.summaryCard, pressed && item.status && styles.buttonPressed]}
                onPress={() => {
                  if (item.status) {
                    setActiveFilter(item.status);
                  }
                }}
              >
          ...
          <View style={styles.progressCard}>
            ...
            <View style={styles.progressTrack}>
              <View style={[styles.progressFill, { width: `${summary.overallPercent}%` }]} />
            </View>
          </View>

            <View style={styles.stageList}>
              {stages.map((stage) => {
                const statusLabel = statusOptions.find((option) => option.value === stage.status)?.label ?? stage.status;
                const statusColors = getStageStatusColors(stage.status);

                return (
                  <Pressable
                    key={stage.id}
                    style={({ pressed }) => [styles.stageCard, { backgroundColor: statusColors.cardBackground }, pressed && styles.buttonPressed]}
                    onPress={() => handleOpenEdit(stage)}
                  >
                    <View style={styles.stageHeader}>
                      <View style={styles.stageCopy}>
                        <Text style={styles.stageName}>{stage.name}</Text>
                      <Text style={styles.stageMeta}>
                        {stage.category || "Sem categoria"} • {stage.responsible || "Sem responsavel"}
                      </Text>
                    </View>
                      <View style={[styles.stageStatusPill, { backgroundColor: statusColors.pillBackground }]}>
                        <Text style={[styles.stageStatusText, { color: statusColors.pillText }]}>{statusLabel}</Text>
                      </View>
                    </View>

                  <View style={styles.progressHeader}>
                    <Text style={styles.stageDates}>
                      Prev: {stage.planned_start || "—"} → {stage.planned_end || "—"}
                    </Text>
                    <Text style={styles.stagePercent}>{stage.percent_complete ?? 0}%</Text>
                  </View>

                  <View style={styles.stageProgressTrack}>
                    <View style={[styles.stageProgressFill, { width: `${stage.percent_complete ?? 0}%` }]} />
                  </View>
                </Pressable>
              );
            })}
